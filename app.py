...
def load_data():
    ...
    except Exception as e: print(f"Supabase error: {e}"); return pd.DataFrame()
def get_df():
    now = time.time()
...
def refresh_data():
    ...
    if fresh.empty: return "No data"
    return f"Refreshed: {len(fresh)} rows ({fresh['date'].min().strftime('%b %d, %Y')} to {fresh['date'].max().strftime('%b %d, %Y')})"
...
def get_kpis():
    ...
    date_rng = f"{df['date'].min().strftime('%b %d')} - {df['date'].max().strftime('%b %d, %Y')}"
...
def build_dashboard(period):
    ...
    if 'customer_count' in dff.columns:
        ...
        fig4 = go.Figure(go.Bar(x=avs['avg'],y=avs['complex'],orientation='h',marker_color='#2ecc71',text=[f"${v:.2f}" for v in avs['avg']],textposition='outside',textfont=dict(color='#c8d8f0',size=11)))
    ...
    try:
        ...
        fig5 = go.Figure(go.Heatmap(z=pivot.values,x=pivot.columns.tolist(),y=pivot.index.tolist(),colorscale=[[0,'#c0392b'],[0.8,'#f39c12'],[1,'#2ecc71']],zmid=100,zmin=70,zmax=120,text=[[f"{v:.1f}%" for v in row] for row in pivot.values],texttemplate="%{text}",textfont=dict(color='white',size=12)))
...
def generate_forecast(seg_type, seg_name, horizon):
    ...
    fig.add_trace(go.Scatter(x=fc_dates,y=fc_vals,name=f'{horizon}-Day Forecast',line=dict(color='#c9a84c',width=2.5,dash='dash'),mode='lines+markers',marker=dict(size=4,color='#c9a84c')))
    fig.add_vline(x=last_date,line_dash='dot',line_color='#c9a84c',opacity=0.6)
    fig.update_layout(**dark(f"{seg_name} - {horizon}-Day Forecast",height=480),hovermode='x unified',legend=dict(orientation='h',yanchor='bottom',y=1.02,xanchor='right',x=1,bgcolor='rgba(10,22,40,0.8)',bordercolor='#1a3a6e',borderwidth=1,font=dict(color='#c8d8f0')),margin=dict(l=70,r=30,t=70,b=50))
    total=sum(fc_vals); avg=total/horizon; peak=max(fc_vals); peak_d=fc_dates[fc_vals.index(peak)].strftime('%b %d, %Y')
    summary = f"**{horizon}-Day Forecast**\n\n| Metric | Value |\n|---|---|\n| Total | **${total:,.2f}** |\n| Daily Avg | **${avg:,.2f}** |\n| Peak | **${peak:,.2f}** on {peak_d} |"
    return fig, summary
...
def route_intent(message):
    ...
    if any(x in m for x in ['underperform','below budget','struggling']): 
        ...
        return "Below 80% (7d):\n" + "\n".join([f"- {r.complex}/{r.brand}: {r.ach*100:.1f}%" for r in under.itertuples()])
    if any(x in m for x in ['top complex','best complex']): g=df.groupby('complex')['actual_revenue_usd'].sum(); return f"Top: {g.idxmax()} at {fmt(g.max())}"
    if any(x in m for x in ['top brand','best brand']): g=df.groupby('brand')['actual_revenue_usd'].sum(); return f"Top: {g.idxmax()} at {fmt(g.max())}"
    ...
    if any(x in m for x in ['total revenue','overall']): return f"Total: ${df['actual_revenue_usd'].sum():,.2f}"
    if '7-day' in m or '7 day' in m: ma=df[df['date']>=df['date'].max()-timedelta(days=7)].groupby('date')['actual_revenue_usd'].sum().mean(); return f"7-day avg: ${ma:,.2f}/day"
    for cx in COMPLEXES:
        if cx.lower() in m:
            sub=df[df['complex']==cx]; return f"{cx}: ${sub['actual_revenue_usd'].sum():,.2f}"
    for br in BRANDS:
        if br.lower() in m:
            sub=df[df['brand']==br]; return f"{br}: ${sub['actual_revenue_usd'].sum():,.2f}"
    return None
def chat(message, history):
    if not message or not message.strip(): return ""
    data_answer = route_intent(message); df = get_df()
    system = f"You are an intelligence assistant for Savanna QSR Group, Zimbabwe. Data: {len(df)} rows. Revenue: ${df['actual_revenue_usd'].sum():,.2f}. Complexes: {', '.join(COMPLEXES)}. Brands: {', '.join(BRANDS)}. Be concise." if not df.empty else "No data loaded."
...
with gr.Blocks(title="Savanna QSR Intelligence", css=css) as demo:
    gr.HTML(f'''<div style="background:linear-gradient(135deg,#0d1b2a,#1a3a5c);padding:18px 28px 16px;border-radius:12px;margin-bottom:4px;border-left:4px solid #c9a84c;">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
            <div><h1 style="color:#fff;margin:0;font-size:22px;">Savanna QSR Intelligence</h1>
            <p style="color:#aed6f1;margin:4px 0 0;font-size:13px;">Live Operations Intelligence - Zimbabwe</p></div>
            <div style="text-align:right;"><p style="color:#c9a84c;margin:0;font-size:10px;font-weight:700;letter-spacing:2px;">NETRISYL INSIGHTS</p></div></div>
        <div style="display:flex;gap:10px;margin-top:14px;flex-wrap:wrap;">
            <div style="background:rgba(10,22,40,0.7);padding:10px 18px;border-radius:8px;border:1px solid #1a3a6e;text-align:center;"><div style="color:#c9a84c;font-size:18px;font-weight:700;">{total_rev}</div><div style="color:#7fb3d3;font-size:11px;">Total Revenue</div></div>
            <div style="background:rgba(10,22,40,0.7);padding:10px 18px;border-radius:8px;border:1px solid #1a3a6e;text-align:center;"><div style="color:#c9a84c;font-size:18px;font-weight:700;">{ach_pct}</div><div style="color:#7fb3d3;font-size:11px;">Budget Achievement</div></div>
            <div style="background:rgba(10,22,40,0.7);padding:10px 18px;border-radius:8px;border:1px solid #1a3a6e;text-align:center;"><div style="color:#c9a84c;font-size:18px;font-weight:700;">{top_cx}</div><div style="color:#7fb3d3;font-size:11px;">Top Complex</div></div>
            <div style="background:rgba(10,22,40,0.7);padding:10px 18px;border-radius:8px;border:1px solid #1a3a6e;text-align:center;"><div style="color:#c9a84c;font-size:18px;font-weight:700;">{top_br}</div><div style="color:#7fb3d3;font-size:11px;">Top Brand</div></div>
            <div style="background:rgba(10,22,40,0.7);padding:10px 18px;border-radius:8px;border:1px solid #1a3a6e;text-align:center;"><div style="color:#7fb3d3;font-size:16px;font-weight:700;">{date_rng}</div><div style="color:#7fb3d3;font-size:11px;">Data Range</div></div>
        </div></div>''')
    with gr.Tabs():
